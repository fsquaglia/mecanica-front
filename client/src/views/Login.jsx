...
const Login = () => {
  ...
  //----------------------------------------------------------
  return (
    <div className={style.window} style={{ minHeight: "600px" }}>
      <LoginForm handleSignClick={handleSignClick} auth={auth} />
      <div>
        {isModalOpen && (
          <SignWindow
            onClose={handleSignWindowClose}
            openCreateCar={openCreateCar}
          />
        )}
      </div>
      <div>{isCreateCarOpen && <CreateModal closer={closer} />}</div>
    </div>
  );
};
