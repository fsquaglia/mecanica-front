...
postRouter.delete("/categorypost/:idCategoryPost", ps.deleteCategoryPost); //elimina una categoría que no esté asociada a un Post, la recibe como idCategoryPost por params
postRouter.patch("/categorypost/:idCategoryPost", ps.updateCategoryPost); //actualiza una categoría de post, recibe idCategoryPost por params y el nuevo nombre por body como {nameCategoryPost}
...
postRouter.get("/postbycategory/:idCategory", ps.getPostByCategory); //devuelve los post por categoría de acuerdo a lo que se pide por query http://localhost:3001/postbycategory/1?columnorder=datePost&order=DESC (ver detalles en elhandler)
postRouter.post("/post", ps.postPost); //crear un post/tips
postRouter.delete("/post/:id", ps.deletePost); //eliminar un post por idPost
postRouter.patch("/post/:id", ps.updatePost); //actualiza datos de un Post, según idPost. Enviar todo lo demás que se tenga que modificar por body, por ejemplo {titlePost: "Nuevo título", idCategory: 2}
